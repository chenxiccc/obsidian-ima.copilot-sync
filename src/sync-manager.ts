...
export class SyncManager {
	...
	/**
	 * 手动重新下载：以 Node.js https 强制重新获取网页/图片/附件（仅桌面端）
	 * Manual re-download: force re-fetch web/images/attachments via Node.js https (desktop only)
	 *
	 * 与自动同步不同，此方法全部走 Node.js https（不经过 requestUrl），用于修复防盗链导致的下载失败
	 * Unlike auto-sync, this method exclusively uses Node.js https (not requestUrl), for recovering from anti-hotlink download failures
	 */
	public async reDownload(file: TFile): Promise<void> {
		const cache = this.app.metadataCache.getFileCache(file);
		const mediaId = cache?.frontmatter?.['media_id'] as string | undefined;
		if (!mediaId) {
			new Notice('非 IMA 同步文件，无法重新下载 / Not an IMA-synced file');
			return;
		}

		new Notice('正在重新下载... / Re-downloading...');

		try {
			this.rebuildClient();
			if (!this.client) {
				new Notice('未配置 API 凭证，无法重新下载 / API credentials not configured');
				return;
			}
			...
			if (mediaInfo.media_type === 11 && mediaInfo.notebook_ext_info?.notebook_id) {
				// 笔记：重新获取 Markdown 内容
				// Note: re-fetch Markdown content
				const notebookId = mediaInfo.notebook_ext_info.notebook_id;
				const mdContent = await this.client.getNoteContentMarkdown(notebookId);
				const opts = this.buildAttachmentOptions();
				const withImages = await this.imageHandler.processContent(mdContent, file.path, opts, '');
				await this.vault.modify(file, withImages);
				new Notice('重新下载完成 / Re-download complete');
				return;
			}

			if (!mediaInfo.url_info?.url) {
				new Notice('无法获取下载链接 / Cannot get download URL');
				return;
			}

			const { url: downloadUrl, headers: imaHeaders } = mediaInfo.url_info;
			const opts = this.buildAttachmentOptions();

			if (mediaInfo.media_type === 2 || mediaInfo.media_type === 6) {
				// 网页/微信文章：Node.js 获取 HTML → defuddle 转 Markdown → 处理图片
				// Web/WeChat: Node.js fetch HTML → defuddle convert → process images
				const nodeHeaders: Record<string, string> = {
					'User-Agent': CHROME_UA,
					'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
					...imaHeaders,
				...
				const html = await this.fileDownloader.fetchHtmlViaNodeHttps(downloadUrl, nodeHeaders);
				const result = convertHtmlToMarkdown(html, {
					url: downloadUrl,
					contentSelector: mediaInfo.media_type === 6 ? '#js_content' : undefined,
				});

				// 保留原 frontmatter 中的字段
				// Preserve original frontmatter fields
				const origFm = cache?.frontmatter ?? {};
				const fmLines: string[] = [];
				if (origFm['media_id']) fmLines.push(`media_id: "${origFm['media_id']}"`);
				if (origFm['source']) fmLines.push(`source: "${origFm['source']}"`);
				if (result.author) fmLines.push(`author: "${result.author}"`);
				if (result.published) fmLines.push(`published: ${result.published}`);
				if (result.authorUrl) fmLines.push(`authorUrl: "${result.authorUrl}"`);
				const frontmatter = `---\n${fmLines.join('\n')}\n---\n\n`;

				const effectiveTitle = result.title || origFm['title'] || '';
				const content = result.content || `> 无法提取网页正文，请访问原文：[链接](${downloadUrl})`;
				const md = effectiveTitle
					? `${frontmatter}# ${effectiveTitle}\n\n${content}`
					: `${frontmatter}${content}`;

				const withImages = await this.imageHandler.processContent(escapeInlineHash(md), file.path, opts, effectiveTitle);
				await this.vault.modify(file, withImages);
				new Notice('重新下载完成 / Re-download complete');
				return;
			}

			if (mediaInfo.media_type === 9 || FILE_MEDIA_TYPES.has(mediaInfo.media_type)) {
				// 图片/文件：Node.js 强制重新下载
				// Image/File: force re-download via Node.js
				const isImage = mediaInfo.media_type === 9;
				const filename = isImage
					? buildStableFilename(downloadUrl, { titleBase: '', fallbackName: 'file', fallbackExt: '.png' })
					: sanitizeFilename(file.basename);
				const attachmentFolder = normalizePath(`${opts.syncFolder}/_attachments`);
				...
				await this.fileDownloader.downloadViaNodeHttps(downloadUrl, destPath, nodeHeaders);
				new Notice('重新下载完成 / Re-download complete');
				return;
			}

			new Notice(`不支持的媒体类型 / Unsupported media type: ${mediaInfo.media_type}`);
		} catch (err) {
			const msg = err instanceof Error ? err.message : String(err);
			new Notice(`重新下载失败 / Re-download failed: ${msg}`);
		}
	}
}
